/**
 * Created by Administrator on 2016/11/22. 控制器模块（控制器层）
 */
angular
		...
		.controller(
				...
				[
						...
						'msgService',
						'$state',
						function($scope, msgService, $state) {
		...
		.controller(
				...
				[
						...
						'msgService',
						'$stateParams',
						function($scope, msgService, $stateParams) {
							...
							$scope.updateHit = function(index, id) {
								...
								// 重新存储到本地存储
								sessionStorage.setItem('hitIds', JSON
										.stringify(hitIds));
								...
								// 更新后台数据
								msgService.updateHit(id).success(function() {
							...
							$scope.deleteMsg = function(index, id, e) {
								...
								// 调用业务功能删除后台数据
								msgService.deleteMsg(id).success(function() {
									// 前端删除
							...
							$scope.getMsg = function(index, id) {
								...
								msgService.getMsg(id).success(function(data) {
									// 数据绑定
							...
							$scope.updateMsg = function(e) {
								...
								// 后台更新
								msgService.updateMsg($scope.modal).success(
										function(data) {
											layer.msg('修改评论成功！');
		...
		.controller(
				...
				[
						...
						'$stateParams',
						'$state',
						function($scope, msgService, $stateParams, $state) {
							...
							// 调用后台查询数据
							msgService.getMsg($stateParams.id).success(
									function(data) {
										// 绑定数据
							...
							$scope.updateMsg = function(e) {
								...
								// 调用后台更新数据
								msgService.updateMsg($scope.data).success(
										function(data) {
				...
				[
						...
						function($scope, msgService, $stateParams) {
							...
							function getData(pageNum) {
								...
								promise.error(function() {
									layer.closeAll();
									layer.msg('网络连接请求失败10！');
								});
							}

							// 初始化数据（如果有传递的页码，按传递的页码显示，没有则默认第一页）
							getData($stateParams.pageNum ? $stateParams.pageNum
									: 1);
